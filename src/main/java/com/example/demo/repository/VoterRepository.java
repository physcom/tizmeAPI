...
public interface VoterRepository extends JpaRepository<Voter, Long> {
    ...
    @Query(value = "select replace(TRIM(v.last_name),E'\\n','') as lastName, replace(TRIM(v.first_name),E'\\n','') as firstName, replace(TRIM(v.middle_name),E'\\n','') as middleName, u.name as uikTitle from voter v left join uik u on u.id = v.uik_id where u.name LIKE %?1% order by lastName", nativeQuery = true)
